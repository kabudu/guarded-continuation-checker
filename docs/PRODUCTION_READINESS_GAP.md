...
| Cross-platform distribution | Candidate Linux bundle v1 defines static x86_64 musl archives, SPDX 2.3 SBOM, deterministic provenance, offline verification, two-source-path reproducibility, and master-only GitHub attestations | Pass the native hosted reproducibility job and retain a signed candidate; add a justified macOS distribution decision and release-level verification evidence | In progress |
