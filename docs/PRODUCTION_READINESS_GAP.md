...
peak RSS. QatQ 0.3.0 preserves the closed opaque-byte API boundary with
exact-byte containers and a bounded byte-chunk visitor while preserving the
frozen envelope.
...
O2 peaks at 4.39 MiB. Complete accounting now includes preflight, production
and verification: 26,775 O0 and 9,264 O2 decoded transitions. Hosted resource
governance, valid-class RTL composition, hosted reproduction, compatibility
...
The identical-scope maintained comparison now closes its local gate. Pinned
angr independently derives all seven behaviors over the complete 256-input
