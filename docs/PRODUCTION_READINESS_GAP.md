...
| Resource governance | Static dimensions; Rust API applies deadlines, stream/file caps, Unix process groups and non-macOS Unix address-space limits; macOS group regression passes; a Rust 1.97 Linux container proves the configured 128-MiB ceiling; Linux CI runs the same test; metrics record enforced controls and stable failure classes | Obtain green hosted-Linux CI evidence for this branch and retain the regression across supported releases; macOS remains development-only for hard memory evidence | In progress; implementation/local platform evidence closed, hosted CI pending |
