...
| Repeated RTL family evidence | Public Rust APIs now construct a canonical finite BTOR2 family with disjoint state, bind exact component and parameter bytes, independently reconstruct the expanded model, replay bounded SAFE and UNSAFE evidence, govern artifact and verifier work, and select explicit exact fallback without masking invalid family input. A retained 2, 4, and 6-instance negative control shows 77.69% to 84.18% structural-model reduction but identical exact evidence and a 292 to 476-byte complete portfolio premium. | Replace identical bindings and reduced equations with the predeclared authentic independent-channel subsystem; add representative-proof and hidden-coupling controls, maintained monolithic evidence, process resource measurements, hosted cross-platform identity, and external-style acceptance | In progress; structural and fail-closed mechanics closed, proof-reuse benefit unproven |
