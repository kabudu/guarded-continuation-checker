...
| Cross-platform distribution | Candidate Linux bundle v1 defines static x86_64 musl archives, SPDX 2.3 SBOM, deterministic provenance, non-executing offline verification, and two-source-path reproducibility; [signed candidate run 29675023822](https://github.com/kabudu/guarded-continuation-checker/actions/runs/29675023822) passes exact SLSA and SPDX identity policy with [retained evidence](../results/linux-evaluation-candidate-v1.md); the exact crates.io payload packages, verifies, installs offline, and passes CLI capability smoke tests locally; macOS is explicitly development-only because it lacks the supported Linux hard-memory containment boundary, while Windows receives portable API compatibility testing but no production distribution | Reproduce the crate-payload gate in hosted CI and add tagged-release compatibility plus release-level verification evidence | In progress; Linux candidate, platform decision, and local crate-payload gates closed |
| Real product validity | Public synthetic/product-shaped fixtures, a source-bound production-tagged OpenTitan AON timer core, and a second independently maintained Apache-2.0 Caliptra watchdog module under narrow authored wrappers; the Caliptra nine-row local baseline has exact both-answer agreement, independently checked evidence, deterministic regeneration, verified composition, and hostile controls | Hosted reproduction of the second design, multiple public firmware/robotics designs at representative integration scope, plus independent self-service evaluation outcomes | Open; second public embedded-core local mechanism closed |
