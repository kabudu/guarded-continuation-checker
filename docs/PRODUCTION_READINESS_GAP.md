...
| Stable interface | Predicate CLI v1 exposes a strict machine-readable version/limit line; freezes certificate commands, arguments and exit meanings; defines migration, portfolio separation and a multi-release deprecation window; both certificate formats retain product-cohort compatibility tests | Extract a versioned Rust library API from the binary and prove downstream crate compatibility | Closed for predicate CLI v1; Rust API open |
