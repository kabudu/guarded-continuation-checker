...
output=$1
if [[ -e "$output" ]]; then
  echo "multi-successor decode graph output already exists: $output" >&2
  exit 2
fi
mkdir -p "$output"
output=$(CDPATH='' cd -- "$output" && pwd)
image="silkeh/clang@sha256:47a73461b8cfb57f0b22988e69cd57992581a35d1a15bc2220eb3a21ab1fc5d3"
stage=$(mktemp -d "${TMPDIR:-/tmp}/gcc-decode-graph-source.XXXXXX")
cleanup() {
  rm -rf "$stage"
}
...
for classes in 1 2 4 8 16 32; do
  ...
  "$baseline" \
    ...
    "$cohort/multisuccessor-decode-graph-v2.bin" \
...
{
  echo "opentitan-pwm-multisuccessor-decode-graph-v2"
  for classes in 1 2 4 8 16 32; do
