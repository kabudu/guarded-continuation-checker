...
build_model() {
  ...
  canonical_ywmap=${ywmap}.canonical
  ...
  sed "s/^  \"gennerator\": \"Yosys .*\",$/  \"gennerator\": \"Pinned Yosys $expected_yosys\",/" \
    "$ywmap" >"$canonical_ywmap"
  test "$(grep -c "^Generated by pinned Yosys $expected_yosys$" \
    "$canonical_model")" -eq 1 || {
    echo "AIGER generator canonicalisation failed: h${horizon}-${label}" >&2
    exit 1
  }
  test "$(grep -c "^  \"gennerator\": \"Pinned Yosys $expected_yosys\",$" \
    "$canonical_ywmap")" -eq 1 || {
    echo "witness-map generator canonicalisation failed: h${horizon}-${label}" >&2
    exit 1
  }
  mv "$canonical_model" "$model"
  mv "$canonical_ywmap" "$ywmap"
  if [ ! -s "$model" ]; then
