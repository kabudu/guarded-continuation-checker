...
mkdir -p "$output"
printf 'symbolic_input_projection_scaling_schema_version,declared_inputs,projected_inputs,horizon,trials,repeats,reachable_targets,min_workload_speedup,median_workload_speedup,max_workload_speedup,agreement,witness_valid,status\n' > "$output/summary.csv"
...
for horizon in 8 16 32 64; do
    ...
    awk -v h="$horizon" -v t="$trials" -v r="$repeats" -v targets="$targets" '
        ...
        END {
            ...
            printf "1,16,2,%d,%d,%d,%d,%.6f,%.6f,%.6f,true,true,ok\n", h,t,r,targets,values[1],median,values[NR]
        }
    ' "$speeds" >> "$output/summary.csv"
done
printf 'symbolic-input-projection-scaling status=VALID summary=%s\n' "$output/summary.csv"
