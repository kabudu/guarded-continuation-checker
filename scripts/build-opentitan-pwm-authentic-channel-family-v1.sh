...
build_one() {
  local channels=$1
  local output=$2
  local raw=$scratch/pwm-$channels.raw.btor2
  (
    cd "$scratch"
    "$yosys" -Q -q -p "
  ...
  awk -v channels="$channels" -v revision="$expected_yosys" '
    ...
    { sub(/ ; [^;]*[.]sv:.*/, "") }
    { print }
  ' "$raw" >"$scratch/canonical-$channels.btor2"
  (set -C; cp "$scratch/canonical-$channels.btor2" "$output") 2>/dev/null || {
    echo "refusing to overwrite $output" >&2
    exit 2
  }
}
...
build_one 4 "$2"
build_one 6 "$3"
echo "opentitan_pwm_authentic_channel_family_v1=GENERATED channels=2,4,6"
