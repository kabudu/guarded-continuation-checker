...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 YOSYS Z3 GCC_PAIR_COHORT_EXAMPLE OUTPUT_DIRECTORY" >&2
  exit 2
fi
...
[[ -x $yosys && -x $z3 && -x $cohort ]] || {
  echo "Yosys, Z3, and the GCC pair cohort executable must be executable" >&2
  exit 2
}
...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/gcc-pwm-pair-trace-baseline.XXXXXXXX")
...
printf '%s\n' \
  'model,query_id,left,right,relation,length,mask,value,horizon,result,bad_frame' \
  >"$scratch/result/maintained-results.csv"

top=opentitan_pwm_symbolic_class_harness
query_smt="$scratch/query-6.smt2"
transcript="$scratch/transcript-6.txt"
cp "$scratch/6.smt2" "$query_smt"
...
query_id=0
for pair in '0 2' '2 4' '4 0'; do
  read -r left right <<<"$pair"
  for relation in equal different; do
    for shape in '1 1 1 0' '2 3 0 2'; do
      read -r length mask value horizon <<<"$shape"
      printf '(echo "QUERY,symbolic-class-6,%s,%s,%s,%s,%s,%s,%s,%s")\n' \
        "$query_id" "$left" "$right" "$relation" "$length" "$mask" "$value" \
        "$horizon" >>"$query_smt"
      for frame in $(seq "$((length - 1))" "$horizon"); do
        ...
        for bit in $(seq 0 "$((length - 1))"); do
          if (( (mask >> bit) & 1 )); then
            ...
            state=$((frame - bit))
            equality="(= ((_ extract $left $left) (|${top}_n pwm_o| s$state)) ((_ extract $right $right) (|${top}_n pwm_o| s$state)))"
            ...
            printf ' %s' "$observation" >>"$query_smt"
          fi
        done
        printf '))\n(check-sat)\n(pop 1)\n' >>"$query_smt"
      done
...
if ! diff -u "$repo/results/opentitan-pwm-channel-pair-trace-maintained-v1.csv" \
...
printf '%s  symbolic-class-6.smt2\n' \
  "$(sha256_file "$scratch/6.smt2")" >"$scratch/result/smt2-sha256.txt"

mv "$scratch/result" "$output"
echo "opentitan_pwm_channel_pair_trace_maintained_baseline_v1=PASS queries=12 output=$output"
