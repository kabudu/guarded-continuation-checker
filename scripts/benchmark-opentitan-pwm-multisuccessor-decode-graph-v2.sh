...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/gcc-decode-graph-resources.XXXXXX")
cleanup() {
  rm -rf "$scratch"
}
...
for classes in 1 2 4 8 16 32; do
  ...
  for route in graph trace-family; do
    if [ "$route" = graph ]; then
...
echo "multi-successor decode graph resources status=MEASURED trials=$trials output=$output"
