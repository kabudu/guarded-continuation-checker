use guarded_continuation_checker::{
    ...
    compiled_mmio_quotient::{build_live_state_mmio_quotient, verify_live_state_mmio_quotient},
};
...
fn run() -> Result<(), String> {
    ...
    if arguments.len() != 3 {
        return Err("usage: live_state_mmio_quotient FIRMWARE_BIN SYMBOLS_TXT".to_string());
    }
    ...
    let quotient =
        build_live_state_mmio_quotient(&image, symbols).map_err(|error| error.to_string())?;
    let verification = verify_live_state_mmio_quotient(&quotient, &image, symbols)
        .map_err(|error| error.to_string())?;

    println!("live_state_mmio_quotient_version={}", quotient.version);
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("live-state MMIO quotient failed: {error}");
            ExitCode::FAILURE
        }
    }
}
