// Repository-authored one-cycle actuator transport-lag replacement plant.
module WashingPlant(
    ...
    reg pending_intake = 1'b0;
    wire applied_fill = fill_water;
    wire applied_spin = spin;
    wire applied_intake = pending_intake;

    always @(posedge clock) begin
        door_open <= door_event;
        imbalance <= imbalance_event;
        motor_failed <= motor_failure_event;
        pending_intake <= water_intake;
        if (controller_fault)
            water_level <= 2'd0;
    ...
    assign sig_wash_completed = 1'b1;
    assign sig_rinse_completed = 1'b1;
