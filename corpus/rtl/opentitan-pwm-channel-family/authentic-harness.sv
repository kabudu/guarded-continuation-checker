// GCC-authored formal harness for exact OpenTitan PWM transition equations.
// SPDX-License-Identifier: Apache-2.0

module opentitan_pwm_authentic_harness #(
  ...
  always_comb begin
    ...
    for (int unsigned ii = 0; ii < NOutputs; ii++) begin
      // Even and odd channels form two configuration classes. Every channel
      // still owns independent upstream state and register write strobes.
      pwm_en_q[ii] = 1'b1;
      pwm_en_qe[ii] = step_q == (4'd2 + ii);
      invert_q[ii] = ii[0] ^ (ii == 0 && step_q >= 4'd10);
      invert_qe[ii] = ii == 0 && step_q == 4'd10;
      phase_delay_q[ii*16 +: 16] = ii[0] ? 16'd2 : 16'd0;
      phase_delay_qe[ii] = step_q == (4'd2 + ii);
      blink_en_q[ii] = 1'b1;
      blink_en_qe[ii] = step_q == (4'd2 + ii);
      htbt_en_q[ii] = ii[0];
      htbt_en_qe[ii] = step_q == (4'd2 + ii);
      duty_cycle_a_q[ii*16 +: 16] = ii[0] ? 16'd6 : 16'd4;
      duty_cycle_a_qe[ii] = step_q == (4'd2 + ii);
      duty_cycle_b_q[ii*16 +: 16] = ii[0] ? 16'd10 : 16'd8;
      duty_cycle_b_qe[ii] = step_q == (4'd2 + ii);
      blink_param_x_q[ii*16 +: 16] = ii[0] ? 16'd1 : 16'd2;
      blink_param_x_qe[ii] = step_q == (4'd2 + ii);
      blink_param_y_q[ii*16 +: 16] = ii[0] ? 16'd1 : 16'd0;
      blink_param_y_qe[ii] = step_q == (4'd2 + ii);
    end
  end
